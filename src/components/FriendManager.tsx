...
export default function FriendManager({ friends, expenses, onAddFriend, onDeleteFriend }: FriendManagerProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6" id="friend-manager-root">
      ...
      <div className="md:col-span-1 bg-white border border-stone-200 rounded-2xl p-6 shadow-sm self-start">
        <h3 className="text-lg font-bold font-display text-stone-900 mb-4 flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-stone-50 text-stone-900 border border-stone-200 rounded-xl px-4 py-2.5 outline-none focus:border-stone-400 focus:bg-white text-sm transition-all"
            />
          </div>
          ...
          <button
            ...
            className="w-full bg-stone-900 hover:bg-stone-800 text-white font-semibold py-2.5 px-4 rounded-xl text-sm transition-colors cursor-pointer flex items-center justify-center gap-1.5"
          >
      ...
      <div className="md:col-span-2 bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
        ...
        {friends.length === 0 ? (
          <div className="text-center py-12 border-2 border-dashed border-stone-100 rounded-xl space-y-2">
            <Users className="w-10 h-10 text-stone-300 mx-auto" />
            <p className="text-stone-500 text-sm">Your dining group is currently empty.</p>
            <p className="text-xs text-stone-400">Add friends on the left to start dining together!</p>
          </div>
        ) : (
              ...
              {friends.map((friend) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-3.5 border border-stone-150 rounded-xl hover:border-stone-300 transition-colors bg-stone-50/50"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-stone-800 text-sm">{friend.name}</p>
