  ...
  return (
    <div className="space-y-6" id="dashboard-stats-root">
      ...
      {friends.length > 0 ? (
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            {recommendedPayer && onQuickPayClick && (
              <button
                ...
                className="bg-white hover:bg-stone-100 text-stone-900 font-semibold px-5 py-3 rounded-2xl text-sm transition-all duration-200 shadow-md flex items-center gap-2 shrink-0 self-start md:self-center cursor-pointer"
              >
        ...
        <div className="bg-amber-50 text-amber-900 p-6 rounded-2xl border border-amber-200 flex gap-3" id="no-friends-warning">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4" id="stats-grid">
        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm space-y-2" id="stat-total-group">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">Group Spend To Date</span>
            <div className="p-2 bg-emerald-50 text-emerald-700 rounded-xl">
              <DollarSign className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-3xl font-bold font-mono text-stone-900 break-words">
              RM{totalSpend.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
            <p className="text-xs text-stone-500 mt-1">Accumulated across {expenses.length} meals</p>
          </div>
        </div>

        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm space-y-2" id="stat-meals-logged">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">Meals Logged</span>
            <div className="p-2 bg-amber-50 text-amber-700 rounded-xl">
              <Calendar className="w-4 h-4" />
            </div>
          </div>
          <div>
            <div className="text-3xl font-bold font-mono text-stone-900">{expenses.length}</div>
            <p className="text-xs text-stone-500 mt-1">Shared food gatherings logged</p>
          </div>
        </div>

        <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm space-y-2" id="stat-calories">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-stone-500">Estimated Portions</span>
          ...
          <div>
            <div className="text-3xl font-bold font-mono text-stone-900">
              {totalCaloriesVal.toLocaleString()} <span className="text-sm font-sans font-medium text-stone-500">kcal</span>
            </div>
            <p className="text-xs text-stone-500 mt-1">AI-estimated total caloric intake</p>
          </div>
        </div>
      </div>
      ...
      {friends.length > 0 && (
        <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm" id="balance-visualizer-container">
          <h3 className="text-lg font-bold font-display tracking-tight text-stone-900 mb-4 flex items-center gap-2">
            <Users className="w-5 h-5 text-stone-600" />
            Friend Balances & Ledger Ledger
          </h3>
                ...
                <div key={item.friend.id} className="space-y-2" id={`balance-item-${item.friend.id}`}>
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2.5">
                      ...
                      />
                      <span className="font-semibold text-stone-800">{item.friend.name}</span>
                    </div>
                  ...
                  <div className="w-full bg-stone-100 h-2.5 rounded-full overflow-hidden flex relative">
                    {/* Visual center point marker */}
                    <div className="absolute left-1/2 top-0 h-full w-0.5 bg-stone-300 z-10" />
          ...
          <div className="mt-5 pt-4 border-t border-stone-100 flex flex-wrap gap-4 justify-between text-xs text-stone-500">
